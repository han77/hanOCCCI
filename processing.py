...
#-----------------------------------------------------------------------------------------------------------------------------------
'''
original code is GOCI2 processing.py created by Joao Henry Huaman Chinchay.
it is modyfied by Hee-Jeong Han.

Description: Process the GOCI-II data
E-mail: han77@kiost.ac.kr
Created date: May 17, 2023
'''
#-----------------------------------------------------------------------------------------------------------------------------------
import numpy as np
...
class GOCI2():
    ...
    def __str__(self):
        ...
        for key in list(self.__dict__.keys()):
            ...
            elif isinstance(self.__dict__[key],GOCI2):
...
class open_dataset():
    ...
    def dimension(self, parameter):
        ...
        try:
            ...
            return GOCI2({'name':parameter.name, 'size':parameter.size});
    ...
    def image(self, parameter, lonlat='center', domain=None, domain_in_pixels=None, up_level=False, nan_mask=None, delta_index=4, fmt=np.float32):
        ...
        try:
            ...
            try:
                ...
                SatHeight = ds.variables['GOCI2_imager_projection'].perspective_point_height
                SatLon = ds.variables['GOCI2_imager_projection'].longitude_of_projection_origin
                SatSweep = ds.variables['GOCI2_imager_projection'].sweep_angle_axis
                ...
                if lonlat == 'center':
                    ...
                    return GOCI2(dict_Field), dict_Lons, dict_Lats;
                    ...
                    dict_Lons = None
                    dict_Lats = None
                    return GOCI2(dict_Field), None, None;
...
class open_mfdataset():
    ...
    def dimension(self, parameter):
        ...
        return GOCI2({'name':np.array(mfparameter_name), 'size':np.array(mfparameter_size)});
...
def calculate_corners(Lons, Lats, fmt=np.float32):
    ...
    try:
        assert (isinstance(Lons,GOCI2)==isinstance(Lats,GOCI2)==True) or (isinstance(Lons,np.ndarray)==isinstance(Lats,np.ndarray)==True)
    except AssertionError:
        print('\nLons and Lats must be GOCI2 class or numpy.ndarray\n')
        return
    else:
        if isinstance(Lons,GOCI2) and isinstance(Lats,GOCI2):
            Lons = Lons.data
            Lats = Lats.data
...
def create_gridmap(Domain, PixResol=2.0, fmt=np.float32):
    ...
    -------
    Lons : object
        GOCI2 class with the longitudes of gridmap.

    Lats : object
        GOCI2 class with the latitude of gridmap.
...
def locate_files(path, prefix, datetime_ini, datetime_fin, use_parameter='scan_start_time'):
    ...
    It locates GOCI2-16/17 files between initial and final date time.
...
def accumulate_in_gridmap(Lons, Lats, parameter_lon, parameter_lat, parameter_value=None, dx=200, dy=200, dz=200, show_progress=True, fmt=np.float32):
    ...
    try:
        assert (isinstance(parameter_lon,GOCI2) and isinstance(parameter_lat,GOCI2)) or (isinstance(parameter_lon,np.ndarray) and isinstance(parameter_lat,np.ndarray))
    except AssertionError:
        print('\n\tparameter_lon and parameter_lat must be GOCI2 class or np.ndarray\n')
        return
    else:

        try:
            assert (isinstance(Lons,GOCI2) and isinstance(Lats,GOCI2)) or (isinstance(Lons,np.ndarray) and isinstance(Lats,np.ndarray))
        except AssertionError:
            print('\nLons and Lats must be GOCI2 class or numpy.ndarray\n')
            return
        else:

            if isinstance(parameter_lon, GOCI2):
            ...
            try:
                assert isinstance(parameter_value,GOCI2) or isinstance(parameter_value,np.ndarray) or isinstance(parameter_value,type(None))
            except AssertionError:
                print('\nparameter_value must be GOCI2 class, numpy.ndarray or None\n')
                return
            else:
                ...
                if isinstance(parameter_value, GOCI2):
                ...
                if isinstance(Lons, GOCI2)==True and isinstance(Lats, GOCI2)==True:
                    Lons = Lons.data
                    Lats = Lats.data
